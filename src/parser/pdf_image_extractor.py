...
class PDFImageExtractor:
    ...
    def _collect_graphic_regions(
        ...
        for image in page.get_images(full=True):
            ...
            for rect in page.get_image_rects(xref):
                ...
                # Filter out regions that extend far beyond page boundaries
                margin = page_width * 0.05  # Allow 5% margin
                if region[0] < -margin or region[2] > page_width + margin:
                    logger.debug(
                        "Skipping out-of-bounds image region: x0=%.1f, x1=%.1f",
                        region[0],
                        region[2],
                    )
                    continue
        ...
        for drawing in page.get_drawings():
            ...
            # Filter out regions that extend far beyond page boundaries
            # These are likely decorative elements or artifacts
            margin = page_width * 0.05  # Allow 5% margin
            if region[0] < -margin or region[2] > page_width + margin:
                logger.debug(
                    "Skipping out-of-bounds graphic region: x0=%.1f, x1=%.1f, page_width=%.1f",
                    region[0],
                    region[2],
                    page_width,
                )
    ...
    def _is_likely_figure_region(
        ...
        if width < 100 or height < page_height * 0.08:  # 从160降到100
            return False
