...
```bash
python -m src.cli.main clear-cache
python -m src.cli.main pipeline --paper papers/your-paper.pdf --verbose
```

### Need more debugging details

```bash
python -m src.cli.main pipeline --paper papers/your-paper.pdf --verbose --no-clean
```
...
- If documentation and behavior differ, trust `src/cli/main.py`, `src/core/pipeline.py`, and `src/planning/slide_planner.py`.
